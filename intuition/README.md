...
- Select a piece and inspect fully legal moves generated by chess.js
...
## Chess v2 rules engine

Chess v2 now integrates **chess.js 1.4.0** as the legal-rules engine. The browser page imports that pinned version directly.

The rules engine is responsible for:

- Legal move generation
- Check and checkmate
- Absolute pins through legal-move enforcement; direct slider pins are also marked visually
- Kingside and queenside castling
- En passant
- Queen promotion and underpromotion to rook, bishop, or knight
- Stalemate
- Insufficient-material draws
- Threefold repetition detection
- Fifty-move-rule detection
- Undo and move-history state

Dedicated teaching presets demonstrate castling/pins, en passant, promotion/underpromotion, and the fifty-move counter.

### How the neural network and chess engine interact

The two systems have deliberately separate responsibilities:

```text
current chess position
        ↓
chess.js generates ALL LEGAL MOVES
        ↓
each legal move is encoded as 74 values
        ↓
editable PyTorch-style neural network scores the move
        ↓
argmax over the legal move scores
        ↓
MODEL-OPTIMAL LEGAL MOVE
```

An immediate legal checkmate receives terminal priority over a non-mating neural score.

The phrase **model-optimal** is intentional. chess.js is a rules library, not a grandmaster-strength AI engine. The page finds the move that is optimal under the student's current neural-network evaluator; it does not claim that the move is objectively optimal chess in the Stockfish sense.

Changing a weight can therefore change the model-optimal move in real time while chess.js guarantees that the selected move is still legal.

### JSON persistence and chess history

JSON exports now store:

- The initial FEN
- The complete move sequence needed to reconstruct repetition state
- The current FEN
- Network architecture
- Weights
- Biases
- Enabled/disabled connections
- Activation function

Import replays the saved legal move history so repetition and the fifty-move clock remain meaningful.
